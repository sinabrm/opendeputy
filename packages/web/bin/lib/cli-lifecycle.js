...
async function resolveDoctorPortStatuses(options = {}) {
  ...
  if (options.explicitPort) {
    ...
    if (desktopEntry && desktopEntry.port === requestedPort) {
      statuses.push({
        ...
        detail: 'Use a CLI instance port from `opencode serve` for tunneling.',
      });
    ...
    statuses.push({
      ...
      detail: `Start one with \`opencode serve --port ${requestedPort}\`.`,
    });
    return { statuses, availableEntries: [] };
  }
  ...
  if (runningEntries.length === 0) {
    statuses.push({
      ...
      detail: 'Start one with `opencode serve`.',
    });
  }
...
async function discoverRunningInstances(options = {}) {
  ...
  try {
    ...
    for (const file of pidFiles) {
      ...
      // confirms the OpenCode runtime. Cmdline identity alone can match a recycled PID
      // from another OpenCode process on a different port. Try all plausible
