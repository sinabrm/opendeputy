...
export function buildMediaAnalysisArgs({ question, files, workingDirectory }) {
  ...
  const prompt = [
    ...
    "Directly inspect every attached image, screenshot, audio file, video, PDF, or document and answer the question from its observable content.",
...
export function extractOpenCodeError(output) {
  for (const line of output.split(/\r?\n/)) {
    if (!line.trim()) continue;
    try {
      const event = JSON.parse(line);
      const message = event.type === "error" ? event.error?.data?.message || event.error?.message : null;
