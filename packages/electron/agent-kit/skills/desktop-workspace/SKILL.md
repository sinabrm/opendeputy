...
description: Use local OpenDeputy tools for Nemotron Omni image, screenshot, audio, and video understanding; durable SQLite memory; LibreOffice document conversion and previews; offline English or Persian Piper speech; and optional ActivityWatch history. Trigger for analyzing local media, remembering approved facts, reading or converting Office documents, generating spoken audio, or answering a user request from local computer activity history.
---
...
For the content of local PNG/JPEG images, screenshots, WAV/MP3 audio, or MP4 video, call `visual_grounding_analyze_media` with the task-relevant absolute paths and the user's question. Nemotron Omni returns text for the main Muse Spark agent. If it reports that content is inaccessible or a format is unsupported, report the limitation from the tool result. Use local OCR only when the user specifically requests OCR or Nemotron Omni reports that it cannot access image content.
...
For screen inspection and clicking, use `$computer-control`. It tries browser structure or Windows accessibility first, then local OCR/OmniParser, then `nvidia/nvidia/nemotron-3-nano-omni-30b-a3b-reasoning` vision for unresolved screenshot targets. The main agent receives coordinates and should verify the screen after each action.
