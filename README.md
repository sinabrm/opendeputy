...
- Route task-relevant text, code, images, screenshots, audio, video, PDFs, and documents to the hosted Muse Spark model for text-only analysis.
...
OpenDeputy does not bundle model weights. It is configured to use OpenCode's hosted `opencode/muse-spark-1.2-contributor-free` model for normal chat, text, code, visual grounding, and multimodal understanding; other providers can be connected in **Settings → Providers**, with credentials handled by OpenCode. Muse Spark returns text or code and does not generate media. If the installed OpenCode runtime cannot transport an advertised attachment type, OpenDeputy reports that limitation instead of inferring content from the filename.
